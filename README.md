...
**Version**: 2.1.4 | **Last Updated**: 2026-09-22 | [Changelog](CHANGELOG.md)
...
- Reload the integration
- Restart Home Assistant
- **Multi-hub (2+ Homey hubs):** From **2.1.2**, entity unique IDs are migrated when a second real hub is added. Ignored discoveries do not count as hubs (**2.1.3**). **2.1.4** keeps the ids you already have, so an update does not turn working sensors unavailable. If you already have orphans, go to **Settings → Devices & Services → Entities**, filter **Disabled** or **Unavailable**, and remove only those Homey duplicates — keep the live ones. Prefer the entity that has a real state over `unavailable`.
